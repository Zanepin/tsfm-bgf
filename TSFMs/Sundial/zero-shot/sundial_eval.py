...
SUNDIAL_MODEL_ID = "thuml/sundial-base-128m"
...
# ==========================================
# PART 3: Sundial inference wrapper
# ==========================================

def run_sundial_inference_batched(model, X_np, device, horizon, batch_size=32, num_samples=20):
    """
    Batch inference for Sundial
    ...
    for i in range(0, n_samples, batch_size):
        # 1. Prepare data
        batch_input = X_tensor_all[i: i + batch_size]  # (Batch, Time)

        # 2. Instance Normalization (ReVIN style)
        batch_mean = batch_input.mean(dim=1, keepdim=True)
        batch_std = batch_input.std(dim=1, keepdim=True) + 1e-5
        batch_input_norm = (batch_input - batch_mean) / batch_std

        # 3. Predict
        with torch.no_grad():
            forecast_norm = model.generate(
                batch_input_norm,
                max_new_tokens=horizon,
                num_samples=num_samples
            )

            # 4. aggregate (Point Forecasting)
            batch_preds_norm = forecast_norm.mean(dim=1)  # (Batch, Horizon)

            # 5. De-normalization
            batch_preds = batch_preds_norm * batch_std + batch_mean
...
def main():
    print(f"Running Sundial Evaluation ({SUNDIAL_MODEL_ID}) on device: {DEVICE}")

    # 1. Load Sundial model
    print(f"Loading {SUNDIAL_MODEL_ID}...")
    try:
        ...
        model.to(DEVICE)
        model.eval()
        print("Sundial Model loaded successfully.")
    except Exception as e:
    ...
    for ds_name, ds_path in DATASETS.items():
        ...
        for horizon in HORIZONS:
            ...
            for sub_id in tqdm(subject_ids, desc=f"Evaluating {ds_name} (H={horizon_min}m)"):
                ...
                try:
                    # Call Sundial inference function
                    Y_pred = run_sundial_inference_batched(
                        model,
                        X_np,
                        DEVICE,
                        horizon=horizon,
                ...
                # Handle possible NaN (Sundial may occasionally generate unstable values)
                if np.isnan(flat_pred).any():
                    flat_pred = np.nan_to_num(flat_pred, nan=np.nanmean(flat_pred))
    ...
    if final_results:
        ...
        print("\n\n================ SUNDIAL EVALUATION RESULTS ================")
        pd.set_option('display.max_columns', None)
        pd.set_option('display.width', 1000)
        print(df_res.to_string(index=False))

        df_res.to_csv("sundial_glucose_evaluation.csv", index=False)
        print("\nResults saved to sundial_glucose_evaluation.csv")
