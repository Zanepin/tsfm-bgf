...
class AutoARIMAWrapper(BaseModelWrapper):
    ...
    def train(self, dataset_type: str, pred_len: int) -> str:
        # AutoARIMA fits per-subject at evaluation time.
        return "autoarima_dummy_path"
    ...
    def evaluate(self, model_path: str, dataset_type: str, pred_len: int) -> Dict[str, Any]:
        ...
        for sid in common_ids:
            ...
            model = AutoARIMA(
                start_p=mc["start_p"], max_p=mc["max_p"],
                start_q=mc["start_q"], max_q=mc["max_q"],
                max_d=mc["max_d"],
                seasonal=False,
                random_state=42
            )
        ...
        out_csv = os.path.join(current_dir, f"autoarima_subject_metrics_{dataset_type}_pred{pred_len}.csv")
...
if __name__ == "__main__":
    ...
    wrapper = AutoARIMAWrapper(BASE_CONFIG)
    datasets = ["op", "re"]
    pred_lengths = [6, 12]
    all_results = []
    ...
    if all_results:
        ...
        summary_df.to_csv(os.path.join(current_dir, "autoarima_experiment_summary_all.csv"), index=False)
        print("\\n=== TSFMs AutoARIMA benchmark test completed ===")
        print(summary_df)
