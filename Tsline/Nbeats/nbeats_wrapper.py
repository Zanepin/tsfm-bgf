...
class NBEATSWrapper(BaseModelWrapper):
    ...
    def train(self, dataset_type: str, pred_len: int):
        model_name = f"nbeats_{dataset_type}_pred{pred_len}_deterministic"
        ...
        return NBEATSModel.load_from_checkpoint(model_name=model_name, work_dir=model_dir, best=True)
    ...
    def evaluate(self, model, dataset_type: str, pred_len: int) -> Dict[str, Any]:
        ...
        out_csv = os.path.join(current_dir, f"nbeats_subject_metrics_{dataset_type}_pred{pred_len}.csv")
...
if __name__ == "__main__":
    ...
    wrapper = NBEATSWrapper(BASE_CONFIG)
    ...
    if all_results:
        ...
        summary_csv_path = os.path.join(current_dir, "nbeats_experiment_summary_all.csv")
        summary_df.to_csv(summary_csv_path, index=False)
        print("\\n=== TSFMs N-BEATS benchmark test completed ===")
        print(summary_df)
